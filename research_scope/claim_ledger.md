...
|---|---|---|---|
| E1 | tanh shows a transfer-to-starvation crossover in 8/8 seeds, `τ*` mean `1.741`, 95% CI `[1.427, 2.056]` | **EMPIRICAL** | `enl_tanh_crossover-20260823-074607` |
| E2 | Exactly one `d_w` sign change per seed; no oscillation | **EMPIRICAL** | same |
| E3 | The drift-level crossover precedes the outcome-level one by `≈1.06` in `τ` | **EMPIRICAL** | same; `tau_star_response` mean `2.805` |
| E4 | GRU shows transfer throughout: 0/8 crossings, `T_geom` above `S_CE` at every `τ` | **EMPIRICAL** | same. Caveat: the gap is closing (`0.527` vs `0.492`, `d_w = 0.035` and falling), so a longer horizon might cross |
...
| E11 | All five shadow-based methods are statistically indistinguishable on the causal weak gap | **EMPIRICAL** | `e3_cdc_dense_ablation-20260823-090702`; all paired intervals overlap, all `p = 1.48e-08` |
