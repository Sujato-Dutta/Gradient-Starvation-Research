...
\title{From Rate Suppression to Causal Gradient Starvation:\\
A Finite-Width Theory of Recurrent Learning}
\ifdefined\ICLRSubmission
  \author{}
\else
...
question in recurrent binary classifiers through a paired intervention: a both-
feature trajectory is compared with a same-initialization trajectory in which only
the strong input channel is ablated.  The contribution is a finite-width package,
not a claimed general dynamical mean-field theory (DMFT).  We derive exact signed-
logit and response dynamics through the full sample empirical NTK and response--
logit cross-kernel.  Freezing these objects defines a nonlinear logistic tangent
surrogate; continuous Gr\"onwall and experiment-matched discrete secant-kernel
bounds compare it with nonlinear training only under supplied kernel-movement
control.  For arbitrary sign changes, cumulative negative relative drift exceeds
cumulative positive drift exactly when strict outcome suppression occurs.  Under a
single transfer-to-rate-suppression crossing this becomes the tail-area criterion, but
causal starvation additionally requires an independent, preregistered weak-only
first-hit learnability gate.  A noiseless rank-one log drift ratio gives sufficient
conditions for one rate crossing.  In the dense-linear model, the realized
initialization also gives an exact pre-trajectory rate-gap threshold and, from the
shared zero response gap, an existential strict local outcome sign outside a caller
indifference band; no quantitative local horizon is claimed.  Uniform product-ball
...
constants are currently constructed.  We also prove stability of quantitatively
isolated transverse first hits under uniform-error, radius, and mesh hypotheses,
an exact six-scalar zero-lag wide limit for any finite realized recurrent matrix
whose noiseless symmetric reference is suppressed from initialization while its
weak-only trajectory remains learnable, and---when feasible---the fixed-chart,
uncapped, target-attaining minimum-norm property and local one-step bound of
Counterfactual Drift Correction (CDC).

Controlled dense-linear experiments show positive weak-learning AUC gaps in all 32
$\rho$--seed cells per lag at lags 0, 2, and 4 (four strengths by eight seed IDs).
In the final eight-seed-per-architecture nonlinear study, tanh has 3/8 causal
certificates; GRU has 5/8 response crossings but 0/8 causal certificates because
the weak-only model never reaches $\beta=0.5$.  A post-hoc terminal sensitivity
audit, derived without retraining, places tanh $B_W(H)$ in $[4.413,4.676]$ and GRU
in $[0.0828,0.187]$; it is not a replacement for the first-hit gate or an
architecture ranking.  A sealed five-endpoint frozen-kernel pilot fails its
preregistered rule.  Only after that failure, a fresh restricted $4\times8$
data/model-seed factorial classifies response-crossing events in 64/64 records and
drift-crossing events in 60/64, with two-way-bootstrap 95\% lower endpoints 1.0 and
0.8125.  These are one-cell event-classification results, not phase, causal,
learnability, calibrated-time, magnitude, prevalence, or kernel-stability
validation.  All executed empirical evidence is synthetic and seed-limited.  A
semi-real generated-cue intervention and expanded nonlinear/CDC tradeoff study are
frozen but unauthorized and outcome-free; neither contributes an empirical claim.
A general positive-lag optimization-time DMFT remains open.
\end{abstract}
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  its matched counterfactual, so $\Delta(\tau)<0$; and
  \item \emph{causal starvation}: outcome suppression occurs while the weak-only
  counterfactual demonstrably learns under the preregistered first-hit gate.
\end{enumerate}
A transfer-to-starvation trajectory additionally has an initial positive excursion
before outcome suppression.  The distinction is empirical as well as logical: the
final GRU study has response crossings but fails the causal learnability gate in
every seed.

\paragraph{Contributions.}
\begin{enumerate}[leftmargin=*]
  \item We prove exact finite-width logistic response flow and full signed-logit
  empirical-NTK/response dynamics.  A closed projected equation $\dot M=Gg$
  requires exact parameter-independent logit realization; the initialization-
  frozen nonlinear logistic surrogate is exact internally, while comparison with
  a trained nonlinear model is conditional on supplied kernel-movement bounds.
  \item We prove a cumulative drift-balance proposition for arbitrary sign
  changes, with the exact single-crossing tail-area criterion as a corollary.  We
  add a conditional noiseless rank-one rate-crossing criterion and an exact
  realized-initialization dense-linear rate-gap threshold with its existential
  strict local outcome-sign implication.  Independently proved product-ball bounds
  then imply unique crossing, outcome suppression, nondegenerate weak-only
  learnability, or finite-horizon safe transfer; no current component constructs
  these bounds for general positive-lag RNNs.  We also prove stability of
  quantitatively isolated transverse first hits under uniform-error, radius, and
  mesh hypotheses, and an exact zero-lag recurrence-invisible six-scalar wide limit
  for arbitrary finite realized recurrent gain, with immediate deterministic
  suppression and weak-only learnability under the noiseless cue law.
  \item We operationalize the rate--outcome--causal hierarchy through paired
  first-hit, AUC, direct-drift, and finite-step certificates.  A post-hoc terminal
  $\beta$ profile reports threshold sensitivity from existing outcomes without
  replacing the preregistered first-hit gate.
  \item We report the full empirical sequence rather than only positive endpoints:
  corrected dense E1, seed-dependent nonlinear E-NL, a failed five-endpoint E19
  pilot, and the fresh E20 success restricted to drift/response event labels.  The
  archives separate auditable outcomes from historically unreconstructible source.
  \item In the fixed implemented Euclidean/Frobenius chart, we prove CDC's exact
  instantaneous strong-drift preservation, feasible uncapped target-attaining
  minimum-norm property, and local $O(\eta^2)$ one-step bound.  The ablation shows
  this local distinction without establishing broad practical superiority or best
  final retention.
\end{enumerate}
...
analytic $\rho_c$ or universal $(\rho,\mathrm{lag})$ phase boundary is known.  The
exact initialization threshold is realization-dependent at positive lag, and its
...
magnitudes, or prevalence.  The eight-seed nonlinear proportions are descriptive,
not population prevalence.  All executed experiments are controlled and synthetic.
The frozen semi-real and expanded nonlinear/CDC protocols remain unauthorized and
outcome-free, so they add no empirical claim.  The general DMFT solver remains
deliberately unimplemented where proof obligations are unmet.
...
logical separation among rate suppression, outcome suppression, and causally gated
starvation.
...
requires a stronger matched-shadow information setting, and all current evidence is
synthetic.
...
\ifdefined\ICLRSubmission
\section{Paired causal framework}
For condition $C\in\{\Bcond,\Wcond\}$, the both-feature and weak-only models
share initialization, labels, samples, weak coordinates, and noise; only the
strong input channel is ablated under $\Wcond$.  Let $M_w^C(\tau)$ be the common
...
\begin{equation}
 ...
 \label{eq:iclr-gap}
\end{equation}
This pairing separates four claims that are often conflated: $d>0$ is
\emph{relative transfer}; $d<0$ is \emph{rate suppression}; $\Delta<0$ is
\emph{outcome suppression}; and \emph{causal starvation} additionally requires
that the weak-only trajectory first hit a preregistered target $\beta$ by horizon
$H$.  A response crossing without weak-only learnability is therefore descriptive,
...
enables.  With sample Jacobian $J$, empirical NTK $\Theta=JJ^\top$, and response
cross-kernel $c_a=J\nabla M_a$, the corresponding exact dynamics are
$\dot r=\Theta\sigma(-r)/n$ and
$\dot M_a=c_a^\top\sigma(-r)/n$.  Freezing $(\Theta,c_a)$ at initialization
defines a nonlinear logistic tangent surrogate.  Its comparison with nonlinear
training is conditional on supplied kernel-movement bounds; event agreement is not
a substitute for that premise.

\section{From rate suppression to causal starvation}
...
\end{equation}
It allows arbitrary sign changes and immediate suppression.  Under one
$+\!\to\!-$ drift crossing at $\tau_d$, it reduces to the tail-area criterion:
negative post-crossing area must exceed the positive peak area.  Neither result
establishes causal starvation without the independent learnability gate.  The
appendix proves this statement, a conditional rank-one sufficient criterion for a
unique rate crossing, and stability of quantitatively isolated transverse first
...
\end{equation}
This uses the realized initialization, not a trained trajectory or a universal
$(\rho,\mathrm{lag})$ boundary.  Shared initialization has $\Delta(0)=0$, so
$d_0<0$ and $d_0>0$ respectively imply strict local outcome suppression and
transfer for all sufficiently small positive times.  This derivative argument is
existential and gives no quantitative horizon; the executable certificate reports
values within its caller tolerance as boundary/undetermined.  If uniform
product-ball bounds independently give $-\Lambda\le d'\le-\lambda<0$ up to
$h_R=R/S_R$, then $d_0/\Lambda\le\tau_d\le d_0/\lambda$;
...
suppression on that finite horizon.  No current implementation constructs
nonvacuous general positive-lag constants.

Finally, the appendix proves a deterministic finite-horizon wide limit at zero lag,
zero background input, and arbitrary finite realized recurrent gain: all pre-final
states vanish, so $W$ is loss-invisible.  For the explicitly noiseless
...
target.  A general positive-lag optimization-time DMFT remains open.

Counterfactual Drift Correction (CDC) uses the weak-only shadow as a local target.
In the fixed implemented Euclidean/Frobenius parameter chart, let
$a=\nabla m_s$, $p=\nabla m_w$, $v=-\nabla L$, and let $q$ be the projection of
$p$ onto $a^\perp$.  For positive deficit
$\delta=[F_w^{\Wcond}-p\cdot v]_+$, uncapped CDC takes
$v'=v+(\delta/\|q\|^2)q$ when feasible.  It preserves $a\cdot v$ exactly, is the
unique feasible target-attaining minimum-norm correction, and changes the
one-step strong response only by $O(\eta^2)$ under local smoothness.  These are
fixed-chart, full-batch, local statements, not trajectory or practical-superiority
claims.

\section{Experiments and audit protocol}
All claim-bearing experiments are controlled and synthetic.  E1 uses a dense
linear RNN with four strengths and seed IDs 4--11: each lag therefore has 32
$\rho$--seed cells but only eight independent seed IDs.  E-NL evaluates one
$(\rho,\mathrm{lag})=(4,2)$ cell with eight seeds each for tanh and GRU, paired
lockstep training, direct-autograd response drift, horizon $H$, and preregistered
first-hit target $\beta=0.5$.  E3 compares CDC with ERM and seven mitigation arms;
the matched-shadow methods use stronger information and roughly twice the model
work.

Two additional protocols are design artifacts, not results.  A 64-record semi-real
MNIST and FashionMNIST protocol uses real core pixels plus an exactly removable
generated cue.  A separate freeze specifies 192 nonlinear records over prospective
$\beta\in\{0.25,0.5,1,2\}$ and 256 method-records for ERM, CDC, Bloop-style
shadow-target rescue, and PCGrad-style shadow-target rescue.  Both validators report
that execution is not authorized; no dataset was downloaded for the study, no
optimizer was created for a scientific run, and no outcome or E-claim was produced.
The two named comparators are non-canonical style variants pending parity audits.
...
a fresh $4\times8$ crossed data/model-seed factorial per architecture.  Its
10,000-replicate two-way bootstrap resamples the two observed seed axes.  Surviving
manifests, predictions, scores, hashes, and seals make the compact outcomes
auditable, but E19/E20's exact historical dirty source snapshots were never
archived; they are not replayable, and regeneration under current source is a new
study.

\section{Results}
\paragraph{Controlled and nonlinear behavior.}
At lags 0, 2, and 4, E1 has a positive weak AUC gap in all 32 $\rho$--seed cells
per lag; mean gaps are 5.76, 8.93, and 6.11.  Lag-8 rows are not called starvation
because the weak-only model is unlearnable.  In E-NL, tanh has complete causal
certificates in 3/8 seeds.  GRU has drift and response crossings in 5/8 but 0/8
causal certificates because no weak-only run first hits $\beta=0.5$.
...
\paragraph{The negative result is quantitatively legible.}
...
\paragraph{CDC.}
All five shadow-based mitigation arms reduce the dense weak AUC gap from 10.5155
to approximately zero.  CDC alone preserves instantaneous strong drift to
$4.77\times10^{-7}$, but it does not have the best final strong response; this is
evidence for its local theorem, not broad practical superiority.

\section{Limitations and conclusion}
The empirical ceiling is explicit: one controlled task family, eight nonlinear
seeds per architecture, no executed real/semi-real experiment, and no estimate of
event prevalence.  The generated-cue MNIST and FashionMNIST intervention is frozen but
unexecuted because its environment, raw-data/index, selection, committed-source,
and claim-authorization gates are unmet.  The expanded beta/nonlinear/CDC tradeoff
protocol is likewise frozen but unexecuted, lacks canonical baseline parity and
memory telemetry, and contributes no outcome.  The unrun Waterbirds adapter is a
minibatch head-coordinate surrogate outside the full-batch matched-shadow theorem,
and its intercept is not an identified bird-shape response; we make no Waterbirds
claim.  E19/E20 are
integrity-auditable but historically unreplayable, and the frozen surrogate lacks a
proved tanh/GRU kernel-movement certificate.  Proposition~\ref{prop:cross-kernel}
and the cumulative balance are deliberately elementary; their value is the exact
finite-width scope and the separation of rate, outcome, and causal claims.  The
paper's central conclusion is therefore narrow but testable: rate suppression
becomes outcome suppression only after negative relative drift repays the positive
advantage, and causal starvation additionally requires a learnable matched
counterfactual.
\label{page:main-end}

\subsection*{Reproducibility statement}
The appendix states all theorem assumptions and proofs, gives complete synthetic
data-generation, optimization, seed, and censoring protocols, maps claims to
artifacts and executable guards, and reports failed as well as successful frozen
studies.  Resolved configurations, environment records, tracked figures, compact
sealed outcomes, a dependency-free artifact verifier, and the full test suite are
provided as supplementary material.  The supplement explicitly distinguishes
reconstructible current-source derivations, auditable historical outcomes, and
historical runs that cannot be replayed from unavailable source bytes.
...
negative.  We reserve \emph{causal starvation} for outcome suppression when the
weak-only trajectory also reaches a preregistered target $\beta$ within horizon
$H$.  If the weak-only model does not reach $\beta$, an outcome crossing is
descriptive but not a causal-starvation certificate.
...
we use Proposition~\ref{prop:cross-kernel}'s direct autograd drift as primary and report
$R$; projected and matched-state decompositions are diagnostics only.

\section{From rate suppression to outcome suppression and causal starvation}
...
not force $\Delta$ to return to zero.  The post-crossing negative area must repay
the positive area already accumulated.  Causal \emph{starvation} further requires
the independent weak-only learnability gate.
...
autograd drift.  The frozen-kernel studies use the same task cell.  E3 compares
...
Two prospective protocols are frozen but unexecuted.  The semi-real design plans
64 paired records on MNIST 3-vs-8 and FashionMNIST 0-vs-6 with real core pixels and
an exactly removable generated cue channel.  The expanded design plans 192
nonlinear records over $\beta\in\{0.25,0.5,1,2\}$ and 256 CDC-tradeoff
method-records.  Their validators both report
that execution is not authorized; unresolved data, environment, commit, seed,
telemetry, parity, and claim gates prevented scientific runs.  No optimizer-backed
outcome or empirical claim was generated.  The excluded evaluator-engineering set
and its non-scientific use are documented in Appendix~\ref{app:experiment-audit}.
...
not used to select a confirmatory condition.
...
predating source fingerprinting.

\section{Results}
\label{sec:results}
\subsection{Controlled dense-linear phase grid}
At lags 0, 2, and 4, all 32 $\rho$--seed cells per lag (four strengths by
eight seed IDs) have positive weak AUC gaps.  Averaged across $\rho$, the lag-wise gaps are 5.76, 8.93, and 6.11,
...
$0.5$.  Thus the GRU failure is not marginal at that operating point.  Terminal
reach is not generally interchangeable with the preregistered first-hit event on a
nonmonotone trajectory, and this post-hoc diagnostic supports neither a new
confirmatory endpoint nor an architecture ranking.
...
\subsection{E19 fails broadly; E20 succeeds only on restricted event labels}
...
\paragraph{Theory and causal scope.}
The finite-width cross-kernel identity, exact sample-logit/response dynamics,
initialization-frozen tangent surrogate, conditional continuous/secant-SGD
comparison, dense-linear geometry, cumulative drift balance and single-crossing
corollary, conditional rank-one criterion, transverse hitting-time bound,
recurrence-invisible zero-lag wide limit, and fixed-chart local CDC results are
mathematical statements with proofs.  Their scopes differ.  The cumulative balance
characterizes outcome suppression but does not supply weak-only learnability; the
rank-one criterion addresses rate crossing; the frozen comparison assumes unproved
kernel-movement control; the wide limit requires recurrence-invisible zero lag and
zero background input but permits any finite realized recurrent matrix; and CDC
has a local one-step guarantee, not a trajectory theorem.

\paragraph{Empirical interpretation.}
E1 shows positive weak AUC gaps in all 32 $\rho$--seed cells per lag at lags 0, 2,
and 4, but those cells reuse eight seed IDs.  E-NL contains only eight seeds per
architecture at one synthetic task cell: tanh is seed-dependent, and GRU exhibits
outcome crossings without passing the causal gate.  The post-hoc terminal profile
shows that the GRU failure is not marginal at $\beta=0.5$, but it neither replaces
the first-hit gate nor ranks architectures.  These counts are descriptive, not
prevalence estimates.  The exploratory search found no 4/4 robust cell.

\paragraph{Frozen-surrogate sequence.}
E19 failed its preregistered five-endpoint rule; correctness on other labels cannot
rescue the tanh learnability failure.  E20 was designed only after that rejection
and supports a narrower claim: drift/response crossing-event classification on one
fresh crossed-seed factorial.  Its phase, causal, and learnability labels were
secondary, its time predictions were systematically early, and tanh trajectory
magnitudes were poor.  Manifest integrity proves that frozen inputs were consumed
unchanged; it does not prove kernel stability or surrogate accuracy.

\paragraph{CDC and baselines.}
CDC uniquely preserves the instantaneous strong drift within the tested shadow-
method family, as its theorem predicts.  The ablation does not show a general
practical advantage: weak rescue is numerically similar across shadow arms, and
other arms retain a larger final strong response.  CDC also assumes a constructible
weak-only shadow and roughly doubled model work.  The Bloop-style and PCGrad-style
arms are repository implementations motivated by those methods, not independently
audited replicas.

\paragraph{Open theory and external validity.}
A positive-lag optimization-time recurrent DMFT still requires an effective
process, visible and loss-invisible closure variables, finite-horizon
well-posedness, joint quenched concentration, and discretization/hitting-time
control.  Neither an empirical closure nor event classification supplies these
objects.  An analytic $\rho_c$ and certified or quantitatively accurate tanh/GRU
crossing times therefore remain open.  All citable outcomes remain synthetic; no
real-data or semi-real experiment has been run.  The frozen generated-cue
MNIST and FashionMNIST intervention is defensible at the design level but unauthorized
and outcome-free, and the expanded beta/nonlinear/CDC protocol is likewise
unexecuted.  Neither changes the empirical scope.  The unrun Waterbirds adapter is
a minibatch head-coordinate surrogate outside the full-batch matched-shadow
...
but not historically replayable.  Earlier E1, E2, E2-R, exploratory E-NL, and E3
runs share a disclosed dirty-source limitation.

\section{Conclusion}
Gradient starvation requires more than a declining weak-feature rate.  Relative
rate suppression becomes outcome suppression only when cumulative negative drift
exceeds the earlier positive advantage; causal starvation additionally requires
that the matched weak-only trajectory first-hit the learnability target.  Exact
finite-width response dynamics and the general cumulative balance make these
separations formal.  The realized-initialization threshold is exact, but its
positive-lag sign is seed-dependent and its trajectory consequences require
independently proved product-ball constants.  The rank-one, hitting-time,
recurrence-invisible zero-lag, and CDC results state their assumptions explicitly;
the noiseless symmetric zero-lag reference starts suppressed, permits arbitrary
finite recurrent gain, and does not supply a transfer phase.

The empirical sequence supports the same discipline.  Corrected E1 is positive
across 32 $\rho$--seed cells per lag, not 32 independent seeds.  In E-NL, tanh
causal crossover is seed-dependent and GRU outcome crossings fail the learnability
gate; the post-hoc terminal profile strengthens that operating-point diagnosis
without replacing the gate.  E19 remains a failed broad predictor, whereas E20
supports only restricted drift/response crossing-event labels.  CDC demonstrates
its exact local constraint but not broad practical superiority.  With eight-seed
nonlinear evidence, no real-data experiment, unreconstructible historical dirty
source for E19/E20, and no general positive-lag DMFT, general recurrent causal
prediction and calibrated crossover times remain open.  The frozen
semi-real and expanded nonlinear/CDC protocols remain prospective design artifacts
with no outcomes, so they do not alter that conclusion.
...
Tangent factorial & tanh, GRU / 32 & $\rho=4$, lag 2 & 256 & 1000 / .01 & $4\times8$ & crossing-event audit \\
E3 & dense linear / 96 & $\rho=4$, lag 2 & 256 & 500 / .01 & 40--47 & CDC ablation \\
\bottomrule
\end{tabular}}
\end{table}
...
The anonymous supplementary archive separates the controlled phase grid,
finite-width reference, exact solver checks, final nonlinear run and terminal
$\beta$ audit, failed broad tangent pilot, restricted tangent factorial, exploratory
negative search, and CDC ablation.  Machine-readable provenance records bind each
claim to its surviving configuration, outcomes, figures, seals, and source-status
classification.  Current-source commands create new runs; they do not relabel
historical dirty-source outcomes as replayable.  In particular, any new frozen-
tangent study must seal a fresh preflight before evaluation, whereas rebuilding the
learnability profile is a read-only derivation from the archived E-NL summary and
